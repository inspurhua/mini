...
select
	case
		when substring(a.attname from '^(.*)_') = 'key' then 'A条件'
		else 'B数据'
	end as type, a.attname as col_name
from
	pg_class as c
...
func QualityInfoUpdate(tenantId, keyId int64, data []bean.QualityInfo) (err error) {
	tx := db.Begin()
	err = tx.Raw(`delete from sys_quality_info where tenant_id = ? and key_id=?`, tenantId, keyId).Error
	if err != nil {
		tx.Rollback()
		return
	}
